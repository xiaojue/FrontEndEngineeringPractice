...
除了构建和模块化标准的一步一步进化，工具上的升级以及针对不同场景下的不同项目的不同模块处理手段。其他方面还有比如前端自动化测试，前端性能和异常监控的升级，比如使用第三方的SDK来收集系统的性能和异常以及自己搭建自己的行为和错误日志等，测试方面也包含了单测框架的升级，功能GUI层面的测试框架演变，还有针对不同终端的测试方案。
